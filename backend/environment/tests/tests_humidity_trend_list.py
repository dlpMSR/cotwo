...
class HumidityTrendListOnSmallSampleTest(TestCase):
    def test_one_record(self):
        ...
        response = self.client.get(reverse('environment:trend.humidity'))
    ...
    def test_response_data_format(self):
        ...
        self.assertEqual(                                     # fromisoformat()で変換したtimestampはdatetimeになる
            type(datetime.fromisoformat(elem['timestamp'])),
            datetime
        )

class HumidityTrendList(TestCase):
    def setUp(self):
    ...
    def test_number_of_data_contained_in_response(self):
        """過去16時間分のデータが存在した時も、trend.humidityは過去12時間分のデータだけを返す。
        """
        response = self.client.get(reverse('environment:trend.humidity'))
