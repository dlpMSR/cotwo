...
export function Co2Chart() {
  ...
  const renderCustomLegendText = (value: string) => {
    return <span style={{ fontSize: 12, color: color.legend }}>{value}</span>;
  };

  return (
    <Box sx={{ width: "100%" }}>
      <ResponsiveContainer width={"100%"} height={255}>
        <LineChart margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
          <XAxis
            dataKey="timestamp"
            type="number"
            domain={["dataMin", "dataMax"]}
            tickCount={12}
            tick={{ stroke: color.tick }}
            tickFormatter={(unixTime) => dayjs.unix(unixTime).format("h A")}
          />
          <YAxis tickCount={10} />
